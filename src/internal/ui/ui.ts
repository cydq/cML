...
export function injectMenu() {
  ...
  menu.appendChild(createSysbox({
    title: 'cML',
    description: 'Hello! This is the cML menu. If you see this, cML is working correctly (hopefully).',
  }))
      ...
      buttons: [
        {
          title: 'Disable',
          action: () => {
            const rest = data.get('enabled')?.filter((s) => s !== mod.name) ?? []
  ...
  for (const name of index.keys()) {
    ...
    available.appendChild(createSysbox({
